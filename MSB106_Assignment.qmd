...
Commune_Data <- inner_join(NOR_KOMM, NOR_KOMM_2, by = "kommunenummer") |>
  ...
  rename("knr" = kommunenummer, "DistMal" = HubDist, "fid" = HubName, "Turnover_capita_retail" = Turnover_capita_retail_Omsetning)
```

```{r}
Commune_Data <- inner_join(Commune_Data, NOR_OSM_SHOP_MAL, by = "fid") |>
  select(knr, DistMal, fid, Turnover_capita_retail, shop, Vinmonopolet) |>
...
```{r}
Dentist_Data <- left_join(Dentist_Data, NOR_OSM_SHOP_MAL, by = "fid") |>
  select(fid, Juridisk.n, Antall.ans, Sum.Drifts, Sum.salgsi, latlong, DistCBD, DistMal, X_sum, DistComp, shop, Vinmonopolet) |>
  rename("SizeMall" = shop, "Winemonopoly" = Vinmonopolet)
```
...
Dentist_Data_LIMIT <- Dentist_Data |>
  filter(Sum.salgsi < 100000000, DistCBD < 250,
         Sum.Drifts < 20000000, Sum.Drifts > -10000000, DistMal < 80)
```
...
```{r}
Dentist_Data |>
  ggplot(aes(x = DistMal, y = Sum.Drifts, colour = DistMal)) +
  geom_point(lwd = .9) +
...
```{r}
huxreg(list("Dentist Sales" = lm1), statistics = c(N = "nobs", R2 = "r.squared"), const. = "(Intercept)", note = "Note: {stars} T statistics in brackets.")
```
